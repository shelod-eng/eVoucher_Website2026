...
export function authorizeSandboxRequest(request: Request) {
  ...
  if (!expectedKey)
    return NextResponse.json({ error: 'Missing SANDBOX_API_KEY on server.' }, { status: 500 });
...
async function resolveCustomer(admin: ReturnType<typeof createAdminClient>, body: AnyRecord) {
  ...
  if (directId)
    return {
      id: directId,
      email: truthyString(body.customer_email) ?? truthyString(body.customerEmail),
    };
  ...
  if (!paymentLookup.error && paymentLookup.data?.[0]?.customer_id) {
    return {
      id: String(paymentLookup.data[0].customer_id),
      email: truthyString(body.customer_email) ?? truthyString(body.customerEmail),
    };
  }
  ...
  if (!voucherLookup.error && voucherLookup.data?.[0]?.customer_id) {
    return {
      id: String(voucherLookup.data[0].customer_id),
      email: truthyString(body.customer_email) ?? truthyString(body.customerEmail),
    };
  }
...
async function resolveMerchant(admin: ReturnType<typeof createAdminClient>, body: AnyRecord) {
  ...
  const requested = (
    truthyString(body.merchant_id) ??
    truthyString(body.merchantId) ??
    truthyString(body.merchant_name) ??
    ''
  ).toLowerCase();
  const match =
    merchants.find((merchant: any) => {
  ...
  return {
    ...
    default_total_discount_pct:
      match.default_total_discount_pct === null ? null : Number(match.default_total_discount_pct),
...
async function dispatchWebhook(reference: string, status: 'completed' | 'failed') {
  ...
  await fetch(
    new URL('/api/payment-callback', process.env.NEXT_PUBLIC_SITE_URL ?? 'http://localhost:4028'),
    {
      method: 'POST',
      headers: {
...
export async function createSandboxPurchase(body: AnyRecord) {
  ...
  const paymentMethod = normalizeSandboxPaymentMethod(
    body.payment_method ?? body.paymentMethod ?? 'card_3ds'
  );
  const faceValue = safeNumber(body.face_value ?? body.faceValue ?? body.amount, 100);
  const totalDiscountPct = safeNumber(
    body.total_discount_pct ?? body.totalDiscountPct,
  ...
  const gatewayFee = round2(
    safeNumber(
      body.gateway_fee ?? body.gatewayFee,
      buildGatewayFee(pricing.consumerPrice, paymentMethod)
    )
  );
  const totalCharge = round2(
    safeNumber(body.total_charge ?? body.totalCharge, pricing.consumerPrice + gatewayFee)
  );
  const transactionReference =
    truthyString(body.ref) ?? truthyString(body.order_ref) ?? generateTransactionReference('SBX');
  ...
  const row = {
    ...
    card_last_four: String(body.card_last_four ?? '1111')
      .slice(-4)
      .padStart(4, '0'),
...
export async function createSandboxTopup(body: AnyRecord) {
  ...
  const customer = await resolveCustomer(admin, body);
  const paymentMethod = normalizeSandboxPaymentMethod(
    body.payment_method ?? body.paymentMethod ?? 'eft'
  );
  ...
  const netAmount = round2(
    safeNumber(
      body.net_amount ?? body.netAmount,
      faceValue - faceValue * ((savingsPct + platformFeePct) / 100)
    )
  );
...
export async function getSandboxPaymentStatus(reference: string) {
  ...
  return {
    ...
    checkout_url:
      paymentStatus === 'pending'
        ? `https://payments.local/checkout/${encodeURIComponent(reference)}`
        : null,
    sandbox: true,
  };
}
export async function addVoucherToWallet(body: AnyRecord) {
  ...
  const merchant = await resolveMerchant(admin, body);
  const voucherCode =
    truthyString(body.voucher_code) ??
    truthyString(body.voucherCode) ??
    generateSecureVoucherCode('WLT');
  const existing = await admin
    .from('customer_vouchers')
    .select('id,voucher_code,face_value,expires_at')
  ...
  if (existing.data?.id) {
    return {
      status: 'active',
      voucher_code: existing.data.voucher_code,
      face_value: Number(existing.data.face_value ?? 0),
      expires_at: existing.data.expires_at ?? null,
      sandbox: true,
      duplicate: true,
    };
  }
...
export async function submitSandboxSettlement(body: AnyRecord) {
  ...
  if (!batchId) {
    ...
    if (invoices.length === 0)
      throw new Error('No approved invoices are available for sandbox settlement submission.');

    const merchantIds = Array.from(
      new Set(invoices.map((invoice: any) => String(invoice.merchant_id)))
    );
    const totalAmount = round2(
      invoices.reduce(
        (sum: number, invoice: any) => sum + Number(invoice.net_payable_to_merchant ?? 0),
        0
      )
    );
    const batchInsert = await admin
      ...
      .insert({
        batch_number: buildBatchNumber(),
        status: 'pending_approval',
        total_amount: totalAmount,
        merchant_count: merchantIds.length,
        notes: 'Created by sandbox gateway settlement submit.',
      })
      .select('*')
      .single();
    if (batchInsert.error || !batchInsert.data)
      throw batchInsert.error ?? new Error('Failed to create settlement batch.');
    batchId = String(batchInsert.data.id);

    const merchantsLookup = await admin
      .from('merchants')
    ...
    const merchantMap = new Map(
      (merchantsLookup.data ?? []).map((entry: any) => [String(entry.id), entry])
    );

    const linkageLookup = await admin
      .from('billing_bank_linkages')
      .select(
        'id,merchant_id,merchant_bank_name,branch_code,account_type,account_holder_name,account_number_enc,account_number_last4,verification_status,is_active'
      )
    ...
    const linkageMap = new Map(
      (linkageLookup.data ?? []).map((entry: any) => [String(entry.merchant_id), entry])
    );
        ...
        status_history_json: [
          {
            ...
            status: 'initiated',
            note: 'Created by sandbox settlement submission.',
          },
        ],
      };
    });
  ...
  if (batchLookup.error || !batchLookup.data)
    throw batchLookup.error ?? new Error('Batch not found.');
  const settlementsLookup = await admin
  ...
  const invoiceIds = Array.from(
    new Set(settlements.map((entry: any) => String(entry.invoice_id)).filter(Boolean))
  );
  const linkageIds = Array.from(
    new Set(settlements.map((entry: any) => String(entry.bank_linkage_id)).filter(Boolean))
  );
  const invoicesLookup = await admin
    .from('billing_invoices')
    .select('id,invoice_number')
    .in('id', invoiceIds);
  if (invoicesLookup.error) throw invoicesLookup.error;
  const linkagesLookup =
    linkageIds.length > 0
      ? await admin
          .from('billing_bank_linkages')
  ...
  const invoiceMap = new Map(
    (invoicesLookup.data ?? []).map((entry: any) => [String(entry.id), entry])
  );
  const linkageMap = new Map(
    (linkagesLookup.data ?? []).map((entry: any) => [String(entry.id), entry])
  );
  ...
  await admin
    .from('billing_settlement_batches')
    .update({ status: 'submitted_to_bank', submitted_at: now })
    .eq('id', batchId);
  await admin
    .from('billing_settlements')
    .update({ status: 'submitted_to_bank', bankserv_batch_id: bankservBatchId, initiated_at: now })
    .eq('batch_id', batchId);
  await admin
    .from('billing_settlement_batches')
  ...
  await admin
    ...
    .update({
      status: 'paid',
      paid_at: now,
      payment_reference: String(batchLookup.data.batch_number ?? '').slice(0, 120) || null,
    })
    .eq('settlement_batch_id', batchId);

  await recordSandboxEvent(admin, {
    eventKey: `sandbox-settlement:${batchId}`,
    eventType: 'manual_adjustment',
    grossAmount: round2(
      settlements.reduce((sum: number, entry: any) => sum + Number(entry.amount ?? 0), 0)
    ),
    occurredAt: now,
    metadata: {
      transactionType: 'settlement_batch',
      batchId,
      batchNumber: batchLookup.data.batch_number,
      bankservBatchId,
  ...
  return {
    batch_id: batchId,
    batch_number: batchLookup.data.batch_number,
    batch_status: 'confirmed',
    bankserv_batch_id: bankservBatchId,
    settlement_count: settlements.length,
    total_amount: round2(
      settlements.reduce((sum: number, entry: any) => sum + Number(entry.amount ?? 0), 0)
    ),
    file_preview: flatFile.split('\n').slice(0, 5),
    sandbox: true,
  };
}
...
export async function refundSandboxTransaction(reference: string, body: AnyRecord) {
  ...
  if (isWalletTopup) {
    await admin.from('wallet_transactions').insert({
      ...
      amount: refundAmount,
      description: `Sandbox refund ${reference}`,
      savings: 0,
    });
  ...
  await recordSandboxEvent(admin, {
    eventKey: `sandbox-refund:${reference}`,
    eventType: 'manual_adjustment',
    merchantId: truthyString(transaction.merchant_id),
    customerId: truthyString(transaction.customer_id),
    grossAmount: refundAmount,
    occurredAt: now,
    metadata: {
      transactionType: 'refund',
      originalReference: reference,
      reason: truthyString(body.reason) ?? 'sandbox_refund',
      isWalletTopup,
    },
  });
  return {
