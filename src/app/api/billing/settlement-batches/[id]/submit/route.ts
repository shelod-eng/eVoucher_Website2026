...
import { formatBankServBatch } from '@/lib/billing/bankserv-formatter';
...
export async function POST(request: Request, context: { params: { id: string } }) {
  ...
  try {
    ...
    const submitRes = await submitBankServBatch(instructions);

    const now = new Date().toISOString();
    await admin
      .from('billing_settlement_batches')
      .update({ status: 'submitted_to_bank', submitted_by: user?.id ?? null, submitted_at: now })
      .eq('id', batchId);
    ...
    await admin
      ...
      .update({
        ...
        bankserv_batch_id: submitRes.batchId,
        initiated_at: now,
      })
      .eq('batch_id', batchId);
    ...
    return jsonNoStore(
      { error: error?.message || 'Failed to submit batch to bank.' },
      { status: 500 }
    );
  }
}
