...
export async function POST(request: Request, context: { params: { id: string } }) {
  try {
    ...
    const { data: batch, error } = await admin
      ...
      .update({ status: 'submitted_to_bank', submitted_by: user.id, submitted_at: submittedAt })
      .eq('id', batchId)
      .select('*')
      .single();

    if (error || !batch) {
      return NextResponse.json({ error: 'Unable to submit batch.' }, { status: 400 });
    }
    ...
    try {
      await writeAuditEvent(admin, {
        ...
        entityId: batchId,
        action: 'settlement_batch_submitted',
    ...
    return NextResponse.json({ message: 'Batch submitted.', batch });
  } catch (error: any) {
    return NextResponse.json(
      { error: error?.message || 'Failed to submit settlement batch.' },
      { status: 500 }
    );
  }
}
