      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <div className="inline-flex items-center space-x-2 px-4 py-2 bg-accent/10 rounded-full mb-6">
            ...
            <span className="text-sm font-headline font-semibold text-accent">Track Your Impact</span>
          </div>
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
            Monitor every rand saved with detailed transaction history and visual analytics. Available via SMS or app.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-12">
          ...
          <div className="bg-gradient-to-br from-success/20 to-success/5 rounded-xl p-6 shadow-lg border-2 border-success/30">
            ...
            <p className="text-4xl font-headline font-bold text-success mb-1">R{totalSavings.toLocaleString()}</p>
          ...
          <div className="bg-gradient-to-br from-primary/20 to-primary/5 rounded-xl p-6 shadow-lg border-2 border-primary/30">
            ...
            <p className="text-4xl font-headline font-bold text-primary mb-1">R{avgMonthlySavings}</p>
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-headline font-semibold transition-all duration-300 ${
                  selectedPeriod === 'quarter' ?'bg-primary text-primary-foreground' :'bg-muted text-muted-foreground hover:bg-muted/80'
                }`}
              >
                Quarter
              </button>
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-headline font-semibold transition-all duration-300 ${
                  selectedPeriod === 'year' ?'bg-primary text-primary-foreground' :'bg-muted text-muted-foreground hover:bg-muted/80'
                }`}
              >
                Year
              </button>
            ...
            <table className="w-full">
              ...
              <tbody>
                {recentTransactions.map((transaction) => (
                  <tr key={transaction.id} className="border-b border-border hover:bg-muted/50 transition-colors duration-200">
                    <td className="py-4 px-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center">
                          <Icon name="BuildingStorefrontIcon" size={20} variant="solid" className="text-primary" />
                        </div>
                        <span className="font-body font-medium text-foreground">{transaction.merchant}</span>
                      </div>
                    </td>
                    <td className="py-4 px-4 text-sm text-muted-foreground">
                      {new Date(transaction.date).toLocaleDateString('en-ZA', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </td>
