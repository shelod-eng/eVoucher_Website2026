...
const ACTIVITY_EVENTS = [
  ...
  {
    time: '10:52 SAST',
    event: 'Bankserv payout ACK received',
    area: 'Settlement',
    severity: 'Normal',
  },
  {
    time: '10:47 SAST',
    event: 'Government food relief campaign activated',
    area: 'Government',
    severity: 'Normal',
  },
  {
    time: '10:40 SAST',
    event: 'Merchant approved: Boxer Gauteng North',
    area: 'Merchants',
    severity: 'Normal',
  },
  {
  ...
  {
    time: '10:21 SAST',
    event: 'Support ticket escalated to treasury',
    area: 'Support',
    severity: 'Medium',
  },
  {
    time: '10:15 SAST',
    event: 'Voucher redeemed via USSD',
    area: 'Voucher Engine',
    severity: 'Normal',
  },
...
const EXECUTIVE_ALERTS = [
  {
    title: 'Government campaign budget threshold reached',
    detail: 'Food relief programme at 92% allocation.',
    action: 'Review budget',
    tone: 'warn',
  },
  {
    title: 'KYC verification backlog',
    detail: '18 merchant records need compliance review.',
    action: 'Open KYC queue',
    tone: 'warn',
  },
  {
    title: 'Fraud pattern detected',
    detail: '3 redemptions blocked across duplicate device fingerprints.',
...
const DEPLOYMENTS = [
  ...
  },
  {
    service: 'USSD API',
    environment: 'Production',
    version: 'ussd-demo-evidence',
    status: 'Ready',
    time: '08:44 SAST',
  },
  {
    service: 'Settlement Cron',
    environment: 'Scheduled',
    version: 'bankserv-acb',
    status: 'Running',
    time: '09:00 SAST',
  },
];
...
const COMPLIANCE_CONTROLS = [
  {
    ...
    evidence: 'Consent, retention, subject access',
  },
  {
    name: 'FICA / KYC',
    status: 'Active',
    owner: 'Compliance',
    evidence: 'Merchant documents and review queues',
  },
  {
    name: 'AML',
    status: 'Monitoring',
    owner: 'Risk',
    evidence: 'Fraud score and redemption rules',
  },
  {
    name: 'PASA',
    status: 'Active',
    owner: 'Treasury',
    evidence: 'Bankserv files, ACK/NCK and settlement audit',
  },
  {
    name: 'PCI DSS',
    status: 'Gateway scoped',
    owner: 'Payments',
    evidence: 'Tokenized payment provider boundary',
  },
  {
    name: 'Audit Logs',
    status: 'Active',
    owner: 'Security',
    evidence: 'Administrative and ledger activity',
  },
];
        ...
        {KPI_WIDGETS.map((metric, index) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                className={`rounded-full border px-2 py-0.5 text-[10px] font-bold uppercase ${toneClasses(metric.tone)}`}
              >
                Live
              </span>
            </div>
            <p className="mt-3 font-headline text-3xl font-bold text-[#20324A]">
              {index < 2
                ? `${Number(metric.value.replace(/,/g, '')) + pulse}`.replace(
                    /\B(?=(\d{3})+(?!\d))/g,
                    ','
                  )
                : metric.value}
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
              <span className="rounded-md bg-[#F7F9FC] px-2 py-1 text-[#16A34A]">
                D {metric.day}
              </span>
              <span className="rounded-md bg-[#F7F9FC] px-2 py-1 text-[#108995]">
                W {metric.week}
              </span>
              <span className="rounded-md bg-[#F7F9FC] px-2 py-1 text-[#64748B]">
                M {metric.month}
              </span>
            </div>
          </div>
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-[#64748B]">
                Chronological platform events refreshing automatically.
              </p>
            </div>
          ...
          <div className="overflow-hidden rounded-lg border border-[#E6EEF5]">
            {ACTIVITY_EVENTS.map((event) => (
              <div
                ...
                className="grid gap-2 border-b border-[#E6EEF5] px-4 py-3 text-sm last:border-b-0 md:grid-cols-[110px_1fr_120px_90px]"
              >
                ...
                <span
                  className={
                    event.severity === 'High' ? 'font-semibold text-[#B91C1C]' : 'text-[#64748B]'
                  }
                >
                  {event.severity}
                </span>
              ...
              <div key={alert.title} className={`rounded-lg border p-4 ${toneClasses(alert.tone)}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <button
                      type="button"
                      className="mt-3 rounded-md bg-white/80 px-3 py-1.5 text-xs font-bold text-[#20324A]"
                    >
                      {alert.action}
                    </button>
                  </div>
      ...
      <div className="rounded-lg border border-[#E6EEF5] bg-white p-5">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-[#64748B]">
              Queues with assign, approve, reject, escalate, and export workflows.
            </p>
          </div>
          <button
            type="button"
          ...
          {OPERATION_QUEUES.slice(0, compact ? 5 : OPERATION_QUEUES.length).map((queue) => (
            <div key={queue.name} className="rounded-lg border border-[#E6EEF5] bg-[#F7F9FC] p-3">
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="rounded-full bg-white px-2.5 py-1 text-xs font-bold text-[#108995]">
                  {queue.count}
                </span>
              </div>
              <div className="mt-2 grid gap-2 text-xs text-[#64748B] sm:grid-cols-3">
                ...
                <button type="button" className="text-left font-bold text-[#108995]">
                  {queue.action}
                </button>
              </div>
            </div>
        ...
        {SYSTEM_HEALTH.map((item) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span
                className={`rounded-full border px-2.5 py-1 text-xs font-bold ${toneClasses(item.tone)}`}
              >
                {item.status}
              </span>
            </div>
      ...
      <div className="rounded-lg border border-[#E6EEF5] bg-white p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          {[
            ...
            <div
              key={item}
              className="flex items-center gap-3 rounded-lg bg-[#F7F9FC] p-3 text-sm font-semibold text-[#20324A]"
            >
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-[#E6EEF5] bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h2 className="font-headline text-xl font-semibold text-[#20324A]">
              CI/CD and Deployments
            </h2>
            <p className="mt-1 text-sm text-[#64748B]">
              GitHub and Vercel promotion status for production services.
            </p>
          </div>
          <a
            ...
            className="inline-flex items-center gap-2 rounded-lg border border-[#20B8C5] bg-white px-4 py-2 text-sm font-semibold text-[#108995] hover:bg-[#EAFBFD]"
          >
      ...
      <div className="overflow-hidden rounded-lg border border-[#E6EEF5] bg-white">
        {DEPLOYMENTS.map((deployment) => (
          <div
            ...
            className="grid gap-2 border-b border-[#E6EEF5] px-5 py-4 text-sm last:border-b-0 md:grid-cols-[1fr_140px_160px_120px_120px]"
          >
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-lg border border-[#E6EEF5] bg-white p-5">
        ...
        <p className="mt-1 text-sm text-[#64748B]">
          Operational controls for POPIA, FICA, KYC, AML, PASA, PCI DSS, audit logs, and regulatory
          reporting.
        </p>
      </div>
        ...
        {COMPLIANCE_CONTROLS.map((control) => (
          ...
          >
            <div className="flex items-start justify-between gap-3">
              ...
              <span className="rounded-full bg-[#EAFBFD] px-3 py-1 text-xs font-bold text-[#108995]">
                {control.status}
              </span>
            </div>
            <p className="mt-4 text-xs font-semibold uppercase tracking-[0.16em] text-[#94A3B8]">
              Owner
            </p>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="rounded-lg border border-[#E6EEF5] bg-white p-4">
            ...
            <div className="mt-3 space-y-2">
              {EXECUTIVE_ALERTS.slice(0, 3).map((item) => (
                <p
                  key={item.title}
                  className="rounded-md bg-[#F7F9FC] px-3 py-2 text-xs text-[#64748B]"
                >
                  {item.title}
                </p>
              ))}
            ...
            <section className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              {STATS.map((stat) => (
                ...
                >
                  ...
                  <p className="mt-2 font-headline text-3xl font-bold text-[#20324A]">
                    {stat.value}
                  </p>
                </div>
              ))}
        ...
        <footer className="border-t border-[#E6EEF5] pt-8 text-center text-sm text-[#64748B]">
          ...
          <p className="mt-2">
            ...
            Vercel Deployments:{' '}
            <a
              href="https://vercel.com/shelod-engs-projects/~/deployments"
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#108995] hover:underline"
            >
