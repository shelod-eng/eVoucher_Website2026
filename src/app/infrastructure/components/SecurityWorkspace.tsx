...
const RBAC_ROLES = [
  ...
  },
  {
    role: 'Finance',
    users: 4,
    permissions: 'Payments, settlements, reports',
    mfa: 'Required',
    status: 'Active',
  },
  {
    ...
    users: 3,
    permissions: 'KYC, audit logs, risk register',
    mfa: 'Required',
    status: 'Active',
  },
  {
    role: 'Merchant Ops',
    users: 6,
    permissions: 'Merchant approvals, documents',
    mfa: 'Enabled',
    status: 'Active',
  },
  {
    role: 'Support Agent',
    users: 12,
    permissions: 'Consumer profiles, tickets',
    mfa: 'Enabled',
    status: 'Active',
  },
  {
    role: 'Auditor',
    users: 3,
    permissions: 'Read-only audit trail',
    mfa: 'Required',
    status: 'Active',
  },
  {
    role: 'Sponsor',
    users: 8,
    permissions: 'Campaign ROI, reports',
    mfa: 'Optional',
    status: 'Active',
  },
  {
    role: 'Government',
    users: 5,
    permissions: 'Programme data, beneficiaries',
    mfa: 'Required',
    status: 'Active',
  },
];
...
const API_KEYS = [
  ...
  {
    id: 'KEY-002',
    name: 'USSD Session Handler',
    scope: 'ussd:session ussd:menu',
    created: '15 Feb 2026',
    expires: '14 Feb 2027',
    status: 'Active',
  },
  {
    id: 'KEY-003',
    name: 'Government Data Feed',
    scope: 'government:read',
    created: '01 Mar 2026',
    expires: '28 Feb 2027',
    status: 'Active',
  },
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              RBAC, MFA enforcement, API keys, failed logins, and threat detection.
            </p>
          </div>
        ...
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="flex items-center justify-between border-b border-[#E6EEF5] p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-[#64748B]">
                Platform roles, user counts, permissions, and MFA enforcement.
              </p>
            </div>
            <button
              type="button"
                ...
                {RBAC_ROLES.map((r) => (
                  <tr key={r.role} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${r.mfa === 'Required' ? 'bg-[#DCFCE7] text-[#166534]' : r.mfa === 'Enabled' ? 'bg-[#EAFBFD] text-[#108995]' : 'bg-[#F1F5F9] text-[#64748B]'}`}
                      >
                        {r.mfa}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusBadge(r.status)}`}
                      >
                        {r.status}
                      </span>
                    </td>
      ...
      {panel === 'logins' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="border-b border-[#E6EEF5] p-5">
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Suspicious authentication events requiring review or escalation.
            </p>
          </div>
          <div className="overflow-x-auto">
              ...
              <tbody>
                {FAILED_LOGINS.map((item) => (
                  <tr key={item.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusBadge(item.action)}`}
                      >
                        {item.action}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        ...
                        <button
                          type="button"
                          className="rounded px-2 py-1 text-xs font-semibold text-[#B91C1C] hover:bg-[#FEE2E2]"
                        >
                          Block IP
                        </button>
                      </div>
        ...
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="flex items-center justify-between border-b border-[#E6EEF5] p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-[#64748B]">
                Platform integration keys with scope, expiry, and rotation controls.
              </p>
            </div>
            <button
              type="button"
              ...
              <tbody>
                {API_KEYS.map((key) => (
                  <tr key={key.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusBadge(key.status)}`}
                      >
                        {key.status}
                      </span>
                    </td>
      ...
      {panel === 'threats' && (
        <div className="space-y-4">
          {THREAT_ALERTS.map((alert) => (
            <div
              key={alert.id}
              className={`rounded-lg border p-5 ${alert.severity === 'High' ? 'border-[#FECACA] bg-[#FEF2F2]' : 'border-[#FDE68A] bg-[#FFFBEB]'}`}
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  <ShieldAlert
                    className={`mt-0.5 h-5 w-5 shrink-0 ${alert.severity === 'High' ? 'text-[#B91C1C]' : 'text-[#92400E]'}`}
                  />
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs text-[#64748B]">{alert.id}</span>
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-bold ${severityBadge(alert.severity)}`}
                      >
                        {alert.severity}
                      </span>
                    </div>
                    ...
                    <p className="text-sm text-[#64748B]">
                      Source: {alert.source} · {alert.time} · {alert.status}
                    </p>
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
                    type="button"
                    className="rounded-lg border border-[#E6EEF5] bg-white px-3 py-1.5 text-xs font-semibold text-[#20324A] hover:bg-[#F7F9FC]"
                  >
