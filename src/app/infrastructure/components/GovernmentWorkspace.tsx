    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Departments, campaigns, budgets, beneficiaries, reporting, and audits.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {(
              [
                ...
                {
                  id: 'approvals',
                  label: `Approvals (${PENDING_APPROVALS.length})`,
                  icon: CheckCircle2,
                },
                { id: 'reports', label: 'Reports', icon: BarChart3 },
              ] as { id: Panel; label: string; icon: typeof Landmark }[]
      ...
      {panel === 'beneficiaries' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="border-b border-[#E6EEF5] p-5">
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Government programme beneficiaries with wallet and status tracking.
            </p>
          </div>
          <div className="overflow-x-auto">
              ...
              <tbody>
                {BENEFICIARIES.map((b) => (
                  <tr key={b.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusBadge(b.status)}`}
                      >
                        {b.status}
                      </span>
                    </td>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#64748B]">
                      Action: {item.action} · {item.dept} · {item.raised}
                    </p>
                  </div>
                </div>
      ...
      {panel === 'reports' && (
        <div className="grid gap-4 sm:grid-cols-2">
          {[
            {
              title: 'Provincial Beneficiary Report',
              desc: 'Breakdown by province, programme, and wallet status.',
              action: 'Export PDF',
            },
            {
              title: 'Budget Utilisation Report',
              desc: 'Budget vs spend per department and programme.',
              action: 'Export Excel',
            },
            {
              title: 'Voucher Redemption Report',
              desc: 'Redemption rates per programme and merchant.',
              action: 'Export CSV',
            },
            {
              title: 'Treasury Audit Pack',
              desc: 'Full settlement and disbursement audit trail.',
              action: 'Export PDF',
            },
          ].map((report) => (
