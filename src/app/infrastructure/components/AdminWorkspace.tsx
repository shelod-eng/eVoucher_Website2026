...
const ADMIN_USERS = [
  {
    ...
    role: 'Super Admin',
    mfa: true,
    lastLogin: '14 Jul 2026 10:02',
    status: 'Active',
  },
  {
    id: 'USR-002',
    name: 'Finance Manager',
    email: 'finance@evoucher.co.za',
    role: 'Finance',
    mfa: true,
    lastLogin: '14 Jul 2026 09:15',
    status: 'Active',
  },
  {
    id: 'USR-003',
    name: 'Compliance Officer',
    email: 'compliance@evoucher.co.za',
    role: 'Compliance Officer',
    mfa: true,
    lastLogin: '13 Jul 2026 16:40',
    status: 'Active',
  },
  {
    id: 'USR-004',
    ...
    role: 'Support Agent',
    mfa: false,
    lastLogin: '14 Jul 2026 08:30',
    status: 'Active',
  },
  {
    id: 'USR-005',
    name: 'Merchant Ops',
    email: 'merchants@evoucher.co.za',
    role: 'Merchant Ops',
    mfa: false,
    lastLogin: '12 Jul 2026 14:22',
    status: 'Inactive',
  },
];
...
const FEATURE_FLAGS = [
  {
    id: 'FF-001',
    name: 'USSD Offline Mode',
    description: 'Allow USSD sessions to cache menus offline',
  ...
  {
    id: 'FF-002',
    name: 'Partial Voucher Redemption',
    description: 'Allow consumers to redeem partial voucher values',
    environment: 'Staging',
    enabled: false,
    staged: true,
  },
  {
    id: 'FF-003',
    name: 'Sponsor Self-Service Portal',
    description: 'Enable sponsors to create campaigns without admin approval',
  ...
  {
    id: 'FF-004',
    name: 'Government Bulk Import',
    description: 'CSV bulk beneficiary import for government programmes',
    environment: 'Production',
    enabled: true,
    staged: false,
  },
  {
    id: 'FF-005',
    name: 'Biometric KYC',
    description: 'Facial recognition for consumer identity verification',
    environment: 'Planned',
    enabled: false,
    staged: false,
  },
...
const EMAIL_TEMPLATES = [
  ...
  },
  {
    id: 'ET-003',
    name: 'KYC Approved',
    trigger: 'Identity verification passed',
  ...
  },
  {
    id: 'ET-005',
    name: 'Fraud Alert',
    trigger: 'Risk score threshold breach',
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Users, roles, feature flags, email templates, and system parameters.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {(
              ...
              ] as { id: Panel; label: string; icon: typeof Users }[]
        ...
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="flex items-center justify-between border-b border-[#E6EEF5] p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-[#64748B]">
                Internal staff accounts with role assignments and MFA status.
              </p>
            </div>
            <button
              type="button"
              ...
              <tbody>
                {ADMIN_USERS.map((u) => (
                  <tr key={u.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${u.mfa ? 'bg-[#DCFCE7] text-[#166534]' : 'bg-[#FEF3C7] text-[#92400E]'}`}
                      >
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusBadge(u.status)}`}
                      >
                        {u.status}
                      </span>
                    </td>
      ...
      {panel === 'flags' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="border-b border-[#E6EEF5] p-5">
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Toggle platform capabilities per environment without code deployments.
            </p>
          </div>
          <div className="divide-y divide-[#E6EEF5]">
            {FEATURE_FLAGS.map((flag) => (
              <div
                key={flag.id}
                className="flex flex-wrap items-center justify-between gap-4 px-5 py-4 hover:bg-[#F7F9FC]"
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${flag.enabled ? 'bg-[#FEE2E2] text-[#B91C1C] hover:bg-[#FECACA]' : 'bg-[#DCFCE7] text-[#166534] hover:bg-[#BBF7D0]'}`}
                  >
        ...
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="flex items-center justify-between border-b border-[#E6EEF5] p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-[#64748B]">
                Transactional and notification email templates with trigger mapping.
              </p>
            </div>
            <button
              type="button"
                  ...
                  <tr key={t.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        ...
                        <button
                          type="button"
                          className="rounded px-2 py-1 text-xs font-semibold text-[#64748B] hover:bg-[#F7F9FC]"
                        >
                          Preview
                        </button>
                      </div>
      ...
      {panel === 'params' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="border-b border-[#E6EEF5] p-5">
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Runtime configuration values controlling platform behaviour.
            </p>
          </div>
          <div className="overflow-x-auto">
                  ...
                  <tr key={p.key} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      {p.editable ? (
                        <button
                          type="button"
                          className="text-xs font-semibold text-[#108995] hover:underline"
                        >
                          Edit value
                        </button>
                      ) : (
