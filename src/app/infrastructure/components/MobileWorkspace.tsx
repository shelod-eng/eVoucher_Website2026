...
const BUILDS = [
  { id: 'BUILD-001', version: 'BuildVersion1', platform: 'Android APK', date: '14 Jul 2026', size: '61 MB', status: 'Released', notes: 'QR scanner, offline voucher caching, USSD fallback' },
  { id: 'BUILD-000', version: 'BuildVersion0', platform: 'Android APK', date: '01 Jun 2026', size: '58 MB', status: 'Superseded', notes: 'Initial release, basic wallet and voucher display' },
];

const PUSH_NOTIFICATIONS = [
  { id: 'PUSH-041', title: 'Voucher Expiry Reminder', audience: 'All consumers', scheduled: '14 Jul 2026 18:00', status: 'Scheduled', sent: '—' },
  { id: 'PUSH-042', title: 'New Merchant: Boxer Gauteng', audience: 'Gauteng consumers', scheduled: '13 Jul 2026 09:00', status: 'Sent', sent: '4,812' },
  { id: 'PUSH-043', title: 'Government Food Relief Available', audience: 'SASSA beneficiaries', scheduled: '01 Jul 2026 08:00', status: 'Sent', sent: '42,000' },
];

const DISTRIBUTION_LINKS = [
  { label: 'Latest APK Download', url: '/downloads/eVoucher_APK_14-July-2026_BuildVersion1.apk', type: 'Direct download' },
  { label: 'Expo EAS Build History', url: 'https://expo.dev/accounts/evoucher_prod/projects/evoucheroffline/builds', type: 'Expo dashboard' },
];
    ...
    <div className="space-y-6">
      <div className="rounded-lg border border-[#E6EEF5] bg-white p-5">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#20B8C5]">Mobile Operations</p>
            <h2 className="mt-1 font-headline text-2xl font-bold text-[#20324A]">Mobile App Management</h2>
            <p className="mt-1 text-sm text-[#64748B]">Android APK releases, Expo EAS builds, push notifications, and distribution.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            ] as { id: Panel; label: string; icon: typeof Smartphone }[]).map(({ id, label, icon: Icon }) => (
              <button
                key={id}
                type="button"
                onClick={() => setPanel(id)}
      ...
      {panel === 'builds' && (
        <div className="space-y-4">
          {BUILDS.map((build) => (
            <div key={build.id} className={`rounded-lg border p-5 ${build.status === 'Released' ? 'border-[#D7F3F6] bg-[#EAFBFD]' : 'border-[#E6EEF5] bg-white'}`}>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <p className="font-headline text-base font-semibold text-[#20324A]">{build.version}</p>
                      <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusBadge(build.status)}`}>{build.status}</span>
                    </div>
                    <p className="text-sm text-[#64748B]">{build.platform} · {build.size} · {build.date}</p>
          ...
          <div className="rounded-lg border border-[#E6EEF5] bg-white p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-[#64748B]">Full build history, logs, and artefacts available on the Expo dashboard.</p>
                <a
      ...
      {panel === 'notifications' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="flex items-center justify-between border-b border-[#E6EEF5] p-5">
            <div>
              <h3 className="font-headline text-lg font-semibold text-[#20324A]">Push Notification Campaigns</h3>
              <p className="mt-1 text-sm text-[#64748B]">Scheduled and sent push notifications with audience targeting.</p>
            </div>
                ...
                {PUSH_NOTIFICATIONS.map((n) => (
                  <tr key={n.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusBadge(n.status)}`}>{n.status}</span>
                    </td>
            ...
            <div key={link.label} className="rounded-lg border border-[#E6EEF5] bg-white p-5">
              <div className="flex flex-wrap items-center justify-between gap-4">
                ...
                <a
                  ...
                  className="inline-flex items-center gap-2 rounded-lg bg-[#20B8C5] px-4 py-2 text-sm font-semibold text-white hover:bg-[#108995]"
                >
                  {link.type === 'Direct download' ? <Download className="h-4 w-4" /> : <ExternalLink className="h-4 w-4" />}
      ...
      {panel === 'history' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="border-b border-[#E6EEF5] p-5">
            ...
            <p className="mt-1 text-sm text-[#64748B]">Chronological record of all mobile app releases and changes.</p>
          </div>
              ...
              <div key={build.id} className="flex items-start gap-4 px-5 py-4">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${statusBadge(build.status)}`}>{build.status}</span>
                  </div>
                  <p className="text-sm text-[#64748B]">{build.date} · {build.platform} · {build.size}</p>
                  <p className="mt-1 text-xs text-[#64748B]">{build.notes}</p>
                </div>
              </div>
            ))}
