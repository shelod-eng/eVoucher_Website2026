...
const SETTLEMENT_BATCHES = [
  {
    id: 'BATCH-2026-07-14-A',
    merchants: 14,
    total: 'R42,400',
    bankserv: 'ACK',
    status: 'Approved',
    processed: '09:00 SAST',
  },
  {
    id: 'BATCH-2026-07-14-B',
    merchants: 8,
    total: 'R18,200',
    bankserv: 'Pending',
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Transaction monitoring, settlement batches, payment providers, and refund management.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {(
              ...
              ] as { id: Panel; label: string; icon: typeof CreditCard }[]
                ...
                {TRANSACTIONS.map((tx) => (
                  <tr key={tx.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${txStatusBadge(tx.status)}`}
                      >
                        {tx.status}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {tx.status === 'Failed' && (
                          <button
                            type="button"
                            className="rounded px-2 py-1 text-xs font-semibold text-[#92400E] hover:bg-[#FEF3C7]"
                          >
                            Refund
                          </button>
                        )}
        ...
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="flex items-center justify-between border-b border-[#E6EEF5] p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-[#64748B]">
                Bankserv ACB file generation and approval workflow.
              </p>
            </div>
            <button
              type="button"
              ...
              <tbody>
                {SETTLEMENT_BATCHES.map((batch) => (
                  <tr key={batch.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${batch.bankserv === 'ACK' ? 'bg-[#DCFCE7] text-[#166534]' : 'bg-[#FEF3C7] text-[#92400E]'}`}
                      >
                        {batch.bankserv}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${batch.status === 'Settled' ? 'bg-[#EAFBFD] text-[#108995]' : batch.status === 'Approved' ? 'bg-[#DCFCE7] text-[#166534]' : 'bg-[#FEF3C7] text-[#92400E]'}`}
                      >
                        {batch.status}
                      </span>
                    </td>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 font-semibold text-[#20324A]">
                      {refund.consumer} — {refund.amount}
                    </p>
                    <p className="text-sm text-[#64748B]">
                      Reason: {refund.reason} · {refund.raised}
                    </p>
                  </div>
                </div>
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    className="rounded-lg bg-[#20B8C5] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#108995]"
                  >
                    Process Refund
                  </button>
                </div>
