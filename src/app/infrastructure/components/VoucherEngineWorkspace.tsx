    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Templates, campaigns, redemption rules, expiry, QR codes, and bulk issuance.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {(
              ...
              ] as { id: Panel; label: string; icon: typeof Gift }[]
              ...
              <tbody>
                {filteredTemplates.map((v) => (
                  <tr key={v.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${statusBadge(v.status)}`}
                      >
                        {v.status}
                      </span>
                    </td>
        ...
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="flex items-center justify-between border-b border-[#E6EEF5] p-5">
            <div>
              ...
              <p className="mt-1 text-sm text-[#64748B]">
                Government and sponsor voucher campaigns with budget tracking.
              </p>
            </div>
            <button
              type="button"
      ...
      {panel === 'rules' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="border-b border-[#E6EEF5] p-5">
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Platform-wide voucher redemption configuration.
            </p>
          </div>
          <div className="overflow-x-auto">
              ...
              <tbody>
                {REDEMPTION_RULES.map((rule) => (
                  <tr key={rule.rule} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${rule.value.includes('Enabled') ? 'bg-[#DCFCE7] text-[#166534]' : rule.value.includes('Disabled') ? 'bg-[#F1F5F9] text-[#64748B]' : 'bg-[#EAFBFD] text-[#108995]'}`}
                      >
                        {rule.value}
                      </span>
                    </td>
      ...
      {panel === 'bulk' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white p-6">
          ...
          <p className="mt-1 text-sm text-[#64748B]">
            Issue vouchers in bulk for government programmes, sponsor campaigns, or corporate
            clients.
          </p>
          ...
          <div className="mt-6 flex gap-3">
            <button
              type="button"
              className="inline-flex items-center gap-2 rounded-lg bg-[#20B8C5] px-5 py-2.5 text-sm font-semibold text-white hover:bg-[#108995]"
            >
              <Gift className="h-4 w-4" /> Generate Vouchers
            </button>
            <button
              type="button"
              className="inline-flex items-center gap-2 rounded-lg border border-[#E6EEF5] bg-white px-5 py-2.5 text-sm font-semibold text-[#20324A] hover:bg-[#F7F9FC]"
            >
