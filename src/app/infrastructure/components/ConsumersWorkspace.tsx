...
const FRAUD_ALERTS = [
  ...
  {
    id: 'FR-043',
    consumer: 'Bongani Zulu',
    type: 'Unusual redemption pattern',
    severity: 'Medium',
    time: '3h ago',
  },
];
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Registration, identity verification, wallet rules, fraud monitoring, and support.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {(
              ...
              ] as { id: Panel; label: string; icon: typeof UserCheck }[]
        ...
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="flex flex-wrap items-center gap-4 border-b border-[#E6EEF5] p-4">
            ...
            <span className="text-sm text-[#64748B]">
              <span className="font-semibold text-[#20324A]">{filtered.length}</span> consumers
            </span>
          </div>
                ...
                {filtered.map((c) => (
                  <tr key={c.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${kycBadge(c.kyc)}`}
                      >
                        {c.kyc}
                      </span>
                    </td>
      ...
      {panel === 'kyc' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="border-b border-[#E6EEF5] p-5">
            ...
            <p className="mt-1 text-sm text-[#64748B]">
              Identity documents awaiting compliance review. Oldest: 6h.
            </p>
          </div>
          <div className="overflow-x-auto">
              ...
              <tbody>
                {KYC_QUEUE.map((item) => (
                  <tr key={item.id} className="border-b border-[#EDF2F7] hover:bg-[#F7F9FC]">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 text-xs font-bold ${item.status === 'Awaiting review' ? 'bg-[#FEF3C7] text-[#92400E]' : 'bg-[#FEE2E2] text-[#B91C1C]'}`}
                      >
                        {item.status}
                      </span>
                    </td>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`rounded-full px-2 py-0.5 text-xs font-bold ${alert.severity === 'High' ? 'bg-[#FEE2E2] text-[#B91C1C]' : 'bg-[#FEF3C7] text-[#92400E]'}`}
                      >
                        {alert.severity}
                      </span>
                    </div>
                    ...
                    <p className="text-sm text-[#64748B]">
                      Consumer: {alert.consumer} · {alert.time}
                    </p>
                  </div>
                </div>
                <div className="flex gap-2">
      ...
      {panel === 'wallet-rules' && (
        <div className="rounded-lg border border-[#E6EEF5] bg-white">
          <div className="border-b border-[#E6EEF5] p-5">
            <h3 className="font-headline text-lg font-semibold text-[#20324A]">
              Wallet Configuration Rules
            </h3>
            <p className="mt-1 text-sm text-[#64748B]">
              Platform-wide wallet limits enforced at transaction level.
            </p>
          </div>
          <div className="overflow-x-auto">
