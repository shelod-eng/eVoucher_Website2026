    ...
    <article className={`group relative overflow-hidden ${BRAND.cardBase} ${BRAND.cardHover}`}>
      ...
      <div className="p-4">
        <div className="mb-3 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-muted-foreground">
              R{Number(voucher.face_value).toFixed(0)} Voucher
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            onClick={() => router.push(`/wallet?code=${voucher.voucher_code}`)}
            className={BRAND.btnOutline + ' py-2 text-xs'}
          >
            View QR
          </button>
          <button
            onClick={() => router.push(`/redeem?code=${encodeURIComponent(voucher.voucher_code)}`)}
  ...
  const recentActivity = useMemo(() => {
    const purchases = paymentTransactions
      ...
      .map((t) => {
        ...
        return {
          id: `p-${t.id}`,
          merchant: mv?.merchant_name ?? 'Voucher Purchase',
          typeLabel: 'Purchase',
      ...
      <PageContent>
        ...
        <PremiumHero
          ...
          stats={[
            ...
            {
              label: 'Total Saved',
              value: `R${rewards.totalCashSaved.toFixed(2)}`,
              sub: 'All time',
            },
            { label: 'Active Vouchers', value: String(activeVouchers.length), sub: 'Ready to use' },
            {
              label: 'Cashback Rate',
              value: `${rewards.savingsRatePct.toFixed(1)}%`,
              sub: 'On every purchase',
            },
          ]}
        />
              ...
              {activeVouchers.length === 0 ? (
                <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-white py-12 text-center">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Visit the shop to buy your first voucher
                  </p>
                  <button
                    onClick={() => router.push('/shop')}
                    className={`mt-4 ${BRAND.btnPrimary}`}
                  >
                    Shop Now
                  </button>
                </div>
