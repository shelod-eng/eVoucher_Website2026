...
export default function ProfilePage() {
  ...
  const sparkMax = useMemo(() => Math.max(...sparkData.map(d => d.spend + d.saved), 1), [sparkData]);
            ...
            {activeTab === 'transactions' && (
              <>
                  ...
                  {(['all', '3m', '6m', '12m'] as const).map(p => (
                    <button key={p} onClick={() => setPeriod(p)}
                      className={`rounded-full px-3 py-1 font-headline text-xs font-bold transition-colors ${
                        period === p ? 'bg-primary text-white' : 'bg-muted text-muted-foreground hover:bg-primary/10'
                      }`}>
                ...
                <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  {[
                    { label: 'Total Spend', value: `R${kpi.spend.toFixed(0)}`, sub: `${filteredTx.length} purchases`, color: 'text-primary' },
                    { label: 'Total Saved', value: `R${kpi.saved.toFixed(0)}`, sub: 'cash benefit', color: 'text-success' },
                    { label: 'Avg per Purchase', value: `R${kpi.avg.toFixed(0)}`, sub: 'per transaction', color: 'text-foreground' },
                    { label: 'Savings Rate', value: `${kpi.rate.toFixed(1)}%`, sub: 'of face value', color: 'text-amber-600' },
                  ].map(tile => (
                    <div key={tile.label} className="rounded-2xl border border-border bg-slate-50 p-4">
                      <p className={`font-headline text-xl font-bold ${tile.color}`}>{tile.value}</p>
                      <p className="mt-0.5 font-headline text-xs font-semibold text-foreground">{tile.label}</p>
                ...
                <div className="mb-4 rounded-2xl border border-border bg-slate-50 p-4">
                  <p className="mb-3 font-headline text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Last 6 months</p>
                      ...
                      return (
                        <div key={d.label} className="flex flex-1 flex-col items-center gap-0.5">
                          <div className="flex w-full items-end gap-px" style={{ height: 64 }}>
                            <div className="flex-1 rounded-t-sm bg-primary/70 transition-all" style={{ height: Math.max(spendH, 2) }} />
                            <div className="flex-1 rounded-t-sm bg-emerald-400/70 transition-all" style={{ height: Math.max(savedH, 2) }} />
                          </div>
                  ...
                  <div className="mt-2 flex gap-4">
                    <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
                      <span className="inline-block h-2 w-2 rounded-sm bg-primary/70" />Spend
                    </span>
                    <span className="flex items-center gap-1 text-[10px] text-muted-foreground">
                      <span className="inline-block h-2 w-2 rounded-sm bg-emerald-400/70" />Saved
                    </span>
                  </div>
                </div>
                ...
                {merchantBreakdown.length > 0 && (
                  <div className="mb-4 rounded-2xl border border-border bg-slate-50 p-4">
                    <p className="mb-3 font-headline text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Top merchants</p>
                    <div className="space-y-2.5">
                      {merchantBreakdown.map((m, i) => (
                        <div key={m.id} className="flex items-center gap-3">
                          <span className="w-4 shrink-0 font-headline text-xs font-bold text-muted-foreground">{i + 1}</span>
                          <div className="flex-1 min-w-0">
                            <div className="mb-1 flex justify-between">
                              <p className="font-headline text-xs font-semibold text-foreground truncate max-w-[65%]">{m.id}</p>
                              <p className="font-headline text-xs font-bold text-primary">R{m.amt.toFixed(0)}</p>
                            </div>
                ...
                {filteredTx.length === 0 ? (
                  ...
                  <div className="relative space-y-0">
                    ...
                    {filteredTx.map((tx, i) => {
                      ...
                      return (
                        <div key={tx.id} className={`relative flex gap-4 pb-4 ${i === filteredTx.length - 1 ? 'pb-0' : ''}`}>
