...
const FraudPrevention = () => {
  ...
  const fraudPreventionMeasures: FraudPreventionMeasure[] = [
    {
      ...
      description: "Real-time analysis of transaction patterns to detect and prevent fraudulent activities",
    ...
    },
    {
      ...
      description: "Secure voucher generation and redemption to prevent duplication and unauthorized use",
      icon: "TicketIcon",
      techniques: [
    ...
    },
    {
      ...
      description: "Comprehensive merchant onboarding and ongoing monitoring to prevent fraudulent merchants",
  ...
  const filteredMeasures = activeCategory === 'all' 
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredMeasures.map((measure) => (
            <div key={measure.id} className="bg-card rounded-lg shadow-md p-6 border border-border hover:shadow-lg transition-shadow duration-300">
              <div className="flex items-start space-x-4 mb-4">
                <div className="flex-shrink-0 w-12 h-12 bg-secondary/10 rounded-lg flex items-center justify-center">
                  <Icon name={measure.icon as any} size={24} variant="solid" className="text-secondary" />
                </div>
                <div className="flex-1">
                  ...
                  <div className="inline-flex items-center space-x-2 px-3 py-1 bg-success/10 rounded-full">
                    <Icon name="CheckBadgeIcon" size={14} variant="solid" className="text-success" />
                    <span className="font-body text-xs font-medium text-success">{measure.effectiveness}</span>
                  </div>
                </div>
              </div>
              
              <div className="mt-4">
                <h4 className="font-body font-semibold text-sm text-foreground mb-3">Prevention Techniques:</h4>
                <ul className="space-y-2">
                  {measure.techniques.map((technique, index) => (
                    <li key={index} className="flex items-start space-x-2">
                      <Icon name="ShieldCheckIcon" size={14} variant="solid" className="text-primary mt-0.5 flex-shrink-0" />
