...
export default function CartPage() {
  ...
  const handleCheckout = () => {
    ...
    // Block checkout if any item is missing a branch (indicates the Boxer→PnP bug scenario)
    const missingBranch = items.find(
      (item) => !item.selectedBranchId && item.redemptionScope !== 'national'
    );
    if (missingBranch) {
      router.push('/shop');
      return;
          ...
          <div className="flex items-center justify-between">
            ...
            {summary.totalSavings > 0 && (
              <div className="rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-right backdrop-blur-sm">
                <p className="font-headline text-xl font-bold text-[#6ee7b7]">
        ...
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-white py-20 text-center shadow-sm">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Browse our merchants and add vouchers to get started
            </p>
            <button
          ...
          <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
            ...
            <div className="space-y-4">
              {items.map((item) => {
                ...
                return (
                  <div
                    key={`${item.merchantId}:${item.productId}:${item.selectedBranchId ?? 'all'}`}
                      ...
                      <div className="mt-4 flex items-center justify-between">
                        <div className="flex items-center gap-1 rounded-xl border border-border bg-muted/30 p-1">
                          <button
                            ...
                            className="flex h-8 w-8 items-center justify-center rounded-lg font-bold text-foreground hover:bg-white"
                          >
                            −
                          </button>
                          <span className="min-w-8 text-center font-headline text-sm font-bold text-foreground">
                            {item.quantity}
                          </span>
                          <button
                            onClick={() =>
              ...
              <div className="p-5 space-y-3">
                ...
                {items.some(
                  ...
                  <div className="rounded-xl border border-amber-300 bg-amber-50 p-3">
                    <p className="font-headline text-xs font-bold text-amber-700">
                      ⚠️ Branch required
                    </p>
                    <p className="mt-0.5 text-xs text-amber-600">
                      One or more items need a branch selected. Return to Shop to fix this before
                      checkout.
                    </p>
                    <button
                      onClick={() => router.push('/shop')}
                      className="mt-2 rounded-lg bg-amber-600 px-3 py-1.5 font-headline text-xs font-bold text-white hover:bg-amber-700"
                    >
                      Go back to Shop
                    </button>
                  </div>
