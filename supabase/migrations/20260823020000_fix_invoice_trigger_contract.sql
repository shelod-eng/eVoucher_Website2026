-- =============================================================================
-- Migration: Repair billing_invoices trigger contract in enqueue_platform_event_outbox()
-- Fixes invalid column references (total_amount, invoice_type) that caused billing_invoices insert errors.
-- =============================================================================
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'billing_invoices' THEN
    IF TG_OP = 'INSERT' THEN
      ...
      v_merchant_id := NEW.merchant_id;
      v_amount := COALESCE(NEW.total_consumer_paid, NEW.consumer_price, NEW.total_face_value, NEW.face_value, 0);
      v_payload := jsonb_build_object('invoice_number', NEW.invoice_number, 'status', NEW.status);
    ELSE
      RETURN NEW;
    END IF;
