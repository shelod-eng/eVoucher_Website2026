...
import { 
  ArrowLeft, DollarSign, CheckCircle2, Clock, XCircle,
  Download, Play, RefreshCw, AlertCircle, FileText, TrendingUp
} from 'lucide-react';
...
export default function SettlementPayouts() {
  ...
  function handleCreateMockBatch() {
    ...
    logAuditEvent('settlement.create_batch', { mode: 'mock', batchNumber: batch.batchNumber, totalAmount: batch.totalAmount });
    refreshMockBatches();
  }
  ...
  function handleApproveMockBatch(batchId) {
    ...
    logAuditEvent('settlement.approve_batch', { mode: 'mock', batchNumber: batch.batchNumber, approvedByEmail });
    refreshMockBatches();
  }
  ...
  function handleExportMockBatch(batch) {
    ...
    logAuditEvent('settlement.export_csv', { mode: 'mock', batchNumber: updated.batchNumber, exportedByEmail });
    refreshMockBatches();
  }
  ...
  function handleSubmitMockBatch(batch) {
    ...
    logAuditEvent('settlement.submit_to_bank', { mode: 'mock', batchNumber: updated.batchNumber, submittedByEmail });
    refreshMockBatches();
  }
  ...
  function handleConfirmMockBatch(batch) {
    ...
    logAuditEvent('settlement.confirmed', { mode: 'mock', batchNumber: updated.batchNumber, confirmedByEmail });
    refreshMockBatches();
  }
  ...
  async function handleCreatePortalBatch() {
    ...
    setPortalNotice('');
    try {
      const notes = session?.email ? `Created via portal by ${session.email}` : 'Created via portal';
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link to={createPageUrl('BillingEngine')}>
              <GoldButton variant="outline" size="sm" className="bg-white/5 border-white/10 text-white hover:bg-white/10">
        ...
        {usePortalApi && portalBatchError ? (
          <Card className="bg-red-500/10 border border-red-500/30 text-red-100 p-3 text-sm">
            {portalBatchError instanceof Error ? portalBatchError.message : 'Failed to load batches.'}
          </Card>
        ) : null}
        ...
        <Card className="bg-white/5 border-white/10 text-white p-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <div className="text-2xl font-bold">R{Number(pendingAmount || 0).toLocaleString()}</div>
              <div className="text-xs text-white/60 mt-1">{pendingCount} merchants ready for batch</div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {batches.length === 0 ? (
              ...
              <div className="space-y-3">
                {batches.map((b) => {
                  ...
                  const canExport = (isFinanceApprover || role === 'admin') && (status === 'approved' || status === 'exported');
                  const canSubmit = (isFinanceApprover || role === 'admin') && status === 'exported';
                  const canConfirm = (isFinanceApprover || role === 'admin') && status === 'submitted_to_bank';
                    ...
                    <div key={batchId} className="rounded-xl border border-white/10 bg-white/5 p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-sm font-bold mt-2">R{Number(totalAmount || 0).toLocaleString()}</div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {canApprove ? (
                          <GoldButton
                            size="sm"
                            onClick={() => (usePortalApi ? handleApprovePortalBatch(batchId) : handleApproveMockBatch(batchId))}
                        ...
                        {canExport ? (
                          <GoldButton
                            size="sm"
                            onClick={() => (usePortalApi ? handleExportPortalBatch(b) : handleExportMockBatch(b))}
                            className="bg-[#00A89D] hover:bg-[#00A89D]/90 text-white"
                          >
                        ...
                        {canSubmit ? (
                          <GoldButton
                            size="sm"
                            onClick={() => (usePortalApi ? handleSubmitPortalBatch(batchId) : handleSubmitMockBatch(b))}
                        ...
                        {canConfirm ? (
                          <GoldButton
                            size="sm"
                            onClick={() => (usePortalApi ? handleConfirmPortalBatch(batchId) : handleConfirmMockBatch(b))}
                      ...
                      <details className="mt-3">
                        ...
                        <div className="mt-2 space-y-2">
                          ...
                          {batchSettlements.map((s) => (
                            <div key={`${batchId}_${s.merchant_id || s.merchantId}`} className="flex items-center justify-between text-xs border border-white/10 bg-white/5 rounded-lg px-2 py-2">
                              <div className="text-white/80">{s.merchant_name || s.merchantName}</div>
                              <div className="font-semibold text-white">R{Number(s.amount || 0).toLocaleString()}</div>
                            </div>
                          ))}
                        </div>
                      </details>
          ...
          <Card className="bg-white/5 border-white/10 text-white p-4">
            ...
            <div className="text-sm text-white/60 space-y-2">
              <div>Next: match settlement confirmations → ledger entries and flag variances.</div>
              <div className="text-xs text-white/50">
                We’ll wire this to Supabase ledger + settlement tables once the backend is ready.
              </div>
            </div>
          </Card>
        </div>
  ...
  const merchantPayouts = merchants.map(merchant => {
        ...
        <div className="px-4 -mt-6">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="w-full bg-white shadow-lg p-1 rounded-xl">
              <TabsTrigger value="pending" className="flex-1 data-[state=active]:bg-[#00A89D] data-[state=active]:text-white rounded-lg">
                Pending
              </TabsTrigger>
              <TabsTrigger value="batches" className="flex-1 data-[state=active]:bg-[#00A89D] data-[state=active]:text-white rounded-lg">
                Batches
              </TabsTrigger>
              <TabsTrigger value="reconcile" className="flex-1 data-[state=active]:bg-[#00A89D] data-[state=active]:text-white rounded-lg">
                Reconcile
              </TabsTrigger>
            </TabsList>
                    ...
                    <Card key={batch.id} className="bg-white border-0 shadow-md p-4">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">{moment(batch.batchDate).format('DD MMM YYYY HH:mm')}</p>
                        </div>
                        <Badge className={
                          ...
                          {batch.status === 'processing' && <RefreshCw className="w-3 h-3 mr-1 animate-spin" />}
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        <div className="bg-gray-50 rounded p-2">
                          ...
                          <p className="text-sm font-semibold text-gray-900">R{batch.totalAmount?.toLocaleString()}</p>
                        </div>
                        <div className="bg-gray-50 rounded p-2">
                      ...
                      <details className="mt-3">
                        ...
                        <div className="mt-2 space-y-2">
                          {batchSettlements.map(settlement => (
                            <div key={settlement.id} className="flex justify-between text-xs bg-gray-50 p-2 rounded">
                              <span className="text-gray-700">{settlement.merchantName}</span>
                              <span className="font-medium text-gray-900">R{settlement.amount?.toLocaleString()}</span>
                            </div>
                          ))}
                        </div>
                      </details>
