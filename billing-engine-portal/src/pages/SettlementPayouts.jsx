...
export default function SettlementPayouts() {
  ...
  const { data: ledgerEntries = [] } = useQuery({
    queryKey: ['allLedger'],
    queryFn: () => (useMock ? Promise.resolve([]) : base44.entities.LedgerEntry.list('-created_date'))
  });
  
  const { data: payoutBatches = [] } = useQuery({
    queryKey: ['payoutBatches'],
    queryFn: () => (useMock ? Promise.resolve([]) : base44.entities.PayoutBatch.list('-created_date'))
  });
        ...
        <Card className="bg-white/5 border-white/10 text-white p-4">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <div className="text-2xl font-bold">R{mockTotalPending.toLocaleString()}</div>
              <div className="text-xs text-white/60 mt-1">{mockPendingSettlements.length} merchants ready for batch</div>
            </div>
            <div className="flex gap-2">
              <GoldButton
                onClick={handleCreateMockBatch}
                disabled={mockPendingSettlements.length === 0}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          <Card className="bg-white/5 border-white/10 text-white p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <Badge className="bg-white/10 border-white/10 text-white">{mockBatches.length}</Badge>
            </div>
            ...
            {mockBatches.length === 0 ? (
              ...
              <div className="space-y-3">
                {mockBatches.map((b) => {
                  const canApprove = isFinanceApprover && b.status === 'pending_approval';
                  const canExport = (isFinanceApprover || role === 'admin') && (b.status === 'approved' || b.status === 'exported');
                  const canSubmit = (isFinanceApprover || role === 'admin') && b.status === 'exported';
                  const canConfirm = (isFinanceApprover || role === 'admin') && b.status === 'submitted_to_bank';
                    ...
                    <div key={b.id} className="rounded-xl border border-white/10 bg-white/5 p-3">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="text-right">
                          <Badge className={statusBadge}>{b.status}</Badge>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {canApprove ? (
                          <GoldButton
                            size="sm"
                            onClick={() => handleApproveMockBatch(b.id)}
                        ...
                        {canExport ? (
                          <GoldButton
                            size="sm"
                            onClick={() => handleExportMockBatch(b)}
                            className="bg-[#00A89D] hover:bg-[#00A89D]/90 text-white"
                          >
                        ...
                        {canSubmit ? (
                          <GoldButton
                            size="sm"
                            onClick={() => handleSubmitMockBatch(b)}
                        ...
                        {canConfirm ? (
                          <GoldButton
                            size="sm"
                            onClick={() => handleConfirmMockBatch(b)}
                      ...
                      <details className="mt-3">
                        ...
                        <div className="mt-2 space-y-2">
                          {(b.settlements || []).map((s) => (
                            <div key={`${b.id}_${s.merchantId}`} className="flex items-center justify-between text-xs border border-white/10 bg-white/5 rounded-lg px-2 py-2">
                              <div className="text-white/80">{s.merchantName}</div>
